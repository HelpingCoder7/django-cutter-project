...
class Auth_viewset(viewsets.ViewSet):    
    ...
    def signupview(self,request):
        ...
        if serializer.is_valid():
            ...
            refresh =RefreshToken.for_user(type("UserObj",(),{"id":str(user["_id"])}))
            return Response({"user":{"username":user['phone_number']},
    ...
    def loginview(self,request):
        ...
        if phone_number == None or password == None:
            return Response({"msg":phone_number})
        else:
            try:
                phone = int(phone_number)  # if stored as int
